...
def to_convex_colliders(mesh: trimesh.Trimesh, threshold: float = 0.1, face_count: Optional[int] = 1000) -> List[trimesh.Trimesh]:
    coacd_mesh = coacd.Mesh(mesh.vertices, mesh.faces)
    convex_meshes = [trimesh.Trimesh(vs, fs) for vs, fs in coacd.run_coacd(coacd_mesh, threshold=threshold)]
    if face_count is not None:
        return [convex_mesh.simplify_quadric_decimation(face_count) for convex_mesh in convex_meshes]
...
def convex_decompose_mesh(object_name: str, resolution: str = "16k", threshold: float = 0.1, face_count: Optional[int] = None) -> None:
    # Infer convex decomposition from mesh.
    mesh = trimesh.load(get_mesh_directory(object_name, resolution) / "centered.obj")
    convex_meshes = to_convex_colliders(mesh, threshold=threshold, face_count=face_count)
...
def convex_decompose_meshes(resolution: str = "16k", threshold: float = 0.1, face_count: Optional[int] = None) -> None:
    for object_name in tqdm(YCB_OBJECT_NAMES, desc="Converting meshes to convex colliders"):
        if pathlib.Path.exists(get_mesh_directory(object_name, resolution) / "centered.obj"):
            convex_decompose_mesh(object_name, resolution=resolution, threshold=threshold, face_count=face_count)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--threshold", type=float, default=0.1, help="Threshold for convex decomposition.")
    parser.add_argument("--face_count", type=int, default=None, help="Target face count for each convex collision mesh.")
    args = parser.parse_args()
    convex_decompose_meshes(resolution=args.resolution, threshold=args.threshold, face_count=args.face_count)
